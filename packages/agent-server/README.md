...
default/plan/acceptEdits 下，`Bash` 的 `can_use_tool` 若命令判定为只读（默认名单 `ls cat head tail wc find grep rg pwd echo stat file which env git`，只认裸命令名，带路径的调用如 `/bin/ls`、`./ls` 一律不判只读；`git` 仅 `status/log/diff/show/rev-parse/branch`，其中 `log/diff/show` 额外拒绝写盘的 `--output`/`--output=<file>`，`branch` 仅纯列出形式（无参数或 `-a/-l/-r/--list/--show-current/-v`）判只读，创建/删除/重命名/设上游等一律走审批；`find` 拒绝 `-delete`、`-exec*`/`-execdir`、`-ok*`/`-okdir`、`-fls`、`-fprint*` 等写标志（前缀族判定）；`env` 仅裸调用或纯 flag；`file` 拒绝 `-C`/`-m`；管道/`&&`/`||`/`;`/单个 `&`（后台）/换行 `\n`、`\r` 串联要求每一段都在名单内；含重定向 `>` `>>` `<`、反引号、`$(...)`、`<(...)`、子 shell `(...)` 一律不判只读），daemon 直接 allow 并以 `thread/engineEvent`（subtype=`readonly_auto_allow`，payload 含 requestId/toolUseId/toolName/command/matchedRules）留痕，同时写入 `approvals` 表一行（`status='auto_allowed'`，`kind='readonly_auto_allow'`），可事后查询是哪条规则放的行；不写入 pendingRequests、不占审批队列；bypass/dontAsk 已有自己的自动应答路径，不受影响，`fileChange`/`item/permissions/requestApproval` 类请求也不受影响。可用 `readonly_auto_allow = false` 整体关闭，或 `readonly_commands = [...]`（整体替换默认名单，非追加）自定义，见下方 `config.toml`。
