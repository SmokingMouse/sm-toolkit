...
describe("in-process JSON-RPC server", () => {
  ...
  test("unattached clients receive no thread notifications and detach preserves live engine", async () => {
    ...
    const { thread } = await a.request("thread/start", { backend: "claude" }); await a.request("turn/start", { threadId: thread.id, input: input("one") }); expect(seen).toEqual([]);
  ...
  test("notification opt-out only affects the declaring connection", async () => {
    ...
    const { thread } = await a.request("thread/start", { backend: "claude" }); await b.request("thread/attach", { threadId: thread.id }); const af = capture(a), bf = capture(b);
  ...
  test("thread and turn keys deduplicate canonical payloads; conflicts are -32013", async () => {
    ...
    const [one, two] = await Promise.all([a.request("thread/start", { backend: "claude", clientThreadId: "thread-key", meta: { a: 1, b: 2 } }), b.request("thread/start", { backend: "claude", clientThreadId: "thread-key", meta: { b: 2, a: 1 } })]);
    expect(one.thread.id).toBe(two.thread.id); expect(two.deduplicated).toBe(true); expect(engines).toHaveLength(1);
    await expect(a.request("thread/start", { backend: "claude", clientThreadId: "thread-key", meta: {} })).rejects.toMatchObject({ code: -32013 });
  ...
  test("input lease excludes others, can be renewed, and disconnect releases", async () => {
    const { server } = create(); const a = await client(server, "a"), b = await client(server, "b"); const { thread } = await a.request("thread/start", { backend: "claude" });
  ...
  test("cwd guard and unsupported backend fail before spawning", async () => {
    ...
    await expect(c.request("thread/start", { backend: "external" })).rejects.toMatchObject({ code: -32008 }); expect(engines).toHaveLength(0);
  });
  ...
  test("reattach on the same connection reissues pending requests with fresh wire IDs", async () => {
    const { server, engines } = create(); const c = await client(server); const frames = capture(c); const { thread } = await c.request("thread/start", { backend: "claude" });
  ...
  test("external backend remains read-only even with an injected engine", async () => {
    const { server } = create({ backends: ["external"], engineFactory: () => new MockEngine(undefined, "external") }); const c = await client(server); const { thread } = await c.request("thread/start", { backend: "external" });
  ...
  test("shutdown rejects new requests while a live engine is closing", async () => {
    const { server, engines } = create(); const c = await client(server); await c.request("thread/start", { backend: "claude" });
    const closing = server.close(); await expect(c.request("thread/start", { backend: "claude" })).rejects.toThrow(); await closing; expect(engines).toHaveLength(1);
  });
  test("MockEngine end-to-end: two clients, approval race, stream, usage, reconnect snapshot", async () => {
    ...
    const { thread } = await a.request("thread/start", { backend: "claude" }); await b.request("thread/attach", { threadId: thread.id }); await readonly.request("thread/attach", { threadId: thread.id });
