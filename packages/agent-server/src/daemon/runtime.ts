...
import { listenCodex, type CodexListener } from "../ingress/codex/listener.js";
...
export async function runDaemon(options: DaemonOptions = {}): Promise<RunningDaemon> {
  ...
  let codexIngress: CodexListener | undefined;
  ...
  try {
    ...
    return { server, manager, paths, webSocketUrl: ws?.url, codexIngressUrl: codexIngress?.url, closed, shutdown(reason = "server_shutdown") {
      if (shuttingDown) return shuttingDown;
      shuttingDown = (async () => {
        log(`agent-server shutting down reason=${reason} graceMs=${graceMs}`);
        try { await server!.close(reason, graceMs); }
        finally {
          unix!.close(); ws?.close(); await codexIngress?.close();
    ...
    unix?.close(); ws?.close(); await codexIngress?.close(); await server?.close(); releasePid(); throw error;
  }
}
