...
import { defaultCodexUnixPath, listenCodex, type CodexListener } from "../ingress/codex/listener.js";

export interface DaemonOptions { paths?: DaemonPaths; graceMs?: number; wsPort?: number; wsAllowedOrigins?: string[]; serverOptions?: ServerOptions; logger?: (message: string) => void; codexTrace?: (direction: "TUI>AS" | "AS>TUI", frame: Record<string, any>, connection?: string) => void }
...
export function readConfig(path: string): ServerOptions & { ws_allowed_origins?: string[]; codex_ingress?: { enabled: boolean; port: number; claude_threads: boolean; unix_path?: string } } {
...
export async function runDaemon(options: DaemonOptions = {}): Promise<RunningDaemon> {
  ...
  let codexIngress: CodexListener | undefined, codexUnix: CodexListener | undefined;
  ...
  try {
    ...
    return { server, manager, paths, webSocketUrl: ws?.url, codexIngressUrl: codexIngress?.url, codexIngressUnixUrl: codexUnix?.url, closed, shutdown(reason = "server_shutdown") {
      ...
      shuttingDown = (async () => {
        ...
        try { await server!.close(reason, graceMs); }
          ...
          unix!.close(); ws?.close(); await codexIngress?.close(); await codexUnix?.close();
    ...
    unix?.close(); ws?.close(); await codexIngress?.close(); await codexUnix?.close(); await server?.close(); releasePid(); throw error;
  }
}
