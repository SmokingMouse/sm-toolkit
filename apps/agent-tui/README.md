...
| `/resume` | — | 打开同一会话选择器 |
| `/resume <完整 thread id>` | — | 恢复指定会话；closed/systemError 时调用 thread/resume 重启引擎，再 attach |
...
切换成功后清理旧会话画面并 detach 旧订阅，不 close/interrupt 旧 thread。Ctrl+N/Ctrl+T 在审批卡上也可使用；卡片保留在 daemon，切回时恢复。快捷键保留原有草稿；命令在请求前清空输入，失败回填。会话操作进行中丢弃新按键并明确提示，完成后重新输入；Ctrl-C 中断/退出仍可用。选择器确认后显示「Esc 不取消在途操作」，此时 Esc 不关闭选择器或假装取消；空列表 Enter 提示按 Esc 退出。

连接中断会显示一行提示，client 自动重连并用 `sinceSeq` attach 回当前 thread，补齐离线完成的 item。daemon 重启后会区分历史恢复与引擎停止，提示用 `/resume` 选择会话；选择或输入 `/resume <id>` 后重启该 thread 引擎并刷新快照，可继续发送 prompt。仍在运行的会话直接 attach，不重复 spawn。
