...
export async function main(args = process.argv.slice(2)): Promise<number> {
  ...
  try {
    ...
    client = new AgentClient(options.endpoint, { token: readToken(options.tokenPath), client: { name: "agent-tui", version: "0.1.0", kind: "tui", label: "agent-tui" }, capabilities: { engineEvents: true, serverRequests: ["item/commandExecution/requestApproval", "item/fileChange/requestApproval", "item/permissions/requestApproval", "item/tool/requestUserInput"] } });
