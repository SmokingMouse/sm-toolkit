...
test("integration ruling: idle takeover expires naturally and cannot renew forever", async () => {
  const f = fixture(80);
  try {
    await f.lease.takeover("th");
    await wait(() => f.model.lease.state === "none");
    const count = f.calls.length;
    expect(count).toBeLessThanOrEqual(3);
    await Bun.sleep(100);
    expect(f.calls).toHaveLength(count);
    expect(f.model.leaseExpiresAt).toBe(0);
    await f.lease.takeover("th");
    const activity = setInterval(() => f.lease.touch("th"), 15);
    try { await Bun.sleep(200); expect(f.model.lease.state).toBe("self"); }
    finally { clearInterval(activity); }
    await wait(() => f.model.lease.state === "none");
  } finally { f.lease.dispose(); }
});
test("integration ruling: switching thread stops old renewal and never reuses its grant", async () => {
  const f = fixture(80);
  try {
    f.model.thread = { id: "old" } as never; f.model.changed();
    await f.lease.takeover("old");
    f.model.thread = { id: "new" } as never; f.model.changed();
    expect(f.model.lease.state).toBe("none"); expect(f.model.leaseExpiresAt).toBe(0);
    const count = f.calls.length;
    await Bun.sleep(140); expect(f.calls).toHaveLength(count);
    await f.lease.run("new", () => {});
    expect(f.calls.slice(count).map(c => [c.method, c.params.threadId])).toEqual([
      ["thread/lease/acquire", "new"], ["thread/lease/release", "new"],
    ]);
  } finally { f.lease.dispose(); }
});
