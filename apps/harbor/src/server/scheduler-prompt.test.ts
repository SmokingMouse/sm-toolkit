...
test("scheduler dispatches wrapped prompt while persisting the raw request", () => {
  ...
  store.setPromptBlock("ws_personal", "session.issue.context", true, "Context={{conversation.id}}", 4);
  store.setPromptBlock("ws_personal", "event.issue.assigned", true, "Request={{prompt}}", 4);
