...
- **Sandbox network access**：Codex Agent 的显式布尔 capability；仅在 `workspace-write` Run 中允许模型进程直接访问网络，dispatch 时冻结进 `RunSpec`。旧 Agent 与缺省值为关闭，read-only 不为联网降级成可写，full 本来绕过 sandbox；Harbor self-deploy coordination 即使 Agent 开网也强制使用断网 outbox + daemon sidecar。它是广域 egress，不是 Git push/merge/deploy 的控制面授权。
