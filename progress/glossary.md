...
- **Delivery provider**：Harbor server 内可替换的 SCM/CD 执行适配器。Provider 负责“怎么向外部系统执行/确认”，Harbor 的 Delivery policy 负责“当前是否允许执行”；`manual` 只记录人工确认，`github` 从 server-only 凭证调用 GitHub REST，两者共享同一 policy 且 manual 始终可独立启动。
