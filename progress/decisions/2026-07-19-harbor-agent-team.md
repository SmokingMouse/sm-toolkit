...
现有 Harbor 已经以 Run 为执行主体、Issue/Delivery 为治理主体，但 Agent 分工主要停留在 instruction：Developer 无法在不拿 owner token 的前提下把 PR 纳入 Delivery，Reviewer 无法把独立 review 转成受 policy 约束的 approve/request-changes/merge；Automation 只有 cron/webhook，也无法可靠表达“进入 Review 就派 Reviewer”“merged 后部署”。
...
- **Deployment Runner**：高权限但私有的技术执行 profile，不承担产品路由。它只由 merged 事件 Automation 调用，更新生产 checkout、验证并重启服务。把部署能力塞进 Orchestrator 会破坏主控的最小权限边界，因此不把它伪装成前三个业务角色之一。
...
- `delivery.merged`：Delivery 首次观察到 merged 时发出；`run + verification` Automation 启动部署。

每个 Trigger 以稳定 `eventId` 持久化去重。server boot 会从已落库的 Run/Issue/Delivery 事实重放缺失事件，解决“事实已提交、进程在 dispatch 前崩溃”的窗口；重复重放不会重复派活。

`source` output 从可信事件的 `conversationId` 动态选择原 Conversation，不允许 schedule/webhook 冒充。Automation 持久化 Run `purpose`，Review 不再伪装成 implementation，部署当前使用 verification purpose。
...
GitHub Delivery 现在允许 `deploymentRequired=true`，推翻旧 ADR 中“GitHub 固定无需部署”的阶段性限制。merged 事件命中部署 Automation 后，Delivery 进入 running；部署 Run succeeded/failed 由 control plane 写回。只有 review approved + checks passed + merged + deployment succeeded 才把 Issue 推进 Done。没有匹配部署 Automation 时保持 pending，不能把“未部署”伪装成完成。

## Rationale

- Agent 负责判断与产出，control plane 负责授权、串行、状态和外部事实；模型失误不能越过 Delivery policy。
- 领域事件比每分钟扫描更及时，也避免重复 Review/部署；boot reconciliation 又补上纯内存 event bus 的丢失窗口。
- PR 创建使用 server credential，Reviewer 使用 scoped decision，不要求在 Device 上散落 GitHub owner token。
- 独立 Deployment Runner 虽然多一个技术 profile，却让 Orchestrator/Reviewer 继续保持最小权限，并让生产命令有单一审计入口。
...
- **cron 每分钟扫描 Review/merged**：拒绝。延迟、重复和 missed-run 语义都比稳定领域事件差；cron 仍保留给真正基于时间的任务。
...
- Deployment Runner 是唯一需要生产 checkout 与服务重启权限的 Agent；必须 private、专仓、单并发，并在 instruction 中要求 clean tree、fast-forward-only、验证通过后再 restart。
- v16 重建 Run prompt-event CHECK、Delivery actor CHECK、Automation/Trigger 表，并保留既有 Run、Delivery event、Automation webhook 去重数据。
