...
## Schema v16 与 migration/bootstrap

v16重建 deployment jobs/gate：新增 fence epoch/nonce、failure kind、target manifest hash、baseline fingerprint/manifest/health hash、per-service PID map；maintenance改为singleton host lock，并新增host epoch high-water row。

迁移只把确有 `deployment_target_id + active job` 的automatic active deployment转为needs recovery。v13/v14 manual/GitHub no-target `running` 保留人工完成语义；v15误迁的这类行按target/job为空和旧migration error精确修复回running。v14空 fingerprint/无 anchor转为legacy ack required。

deploy worker使用`openDeploymentDb()`，只接受exact v16，不创建DB、不运行应用migration。管理员首次启用automatic target必须按一次性bootstrap协议：

1. 停止旧 daemon与server，并证明全部label/PID已退出。
2. 对v13 DB创建0600一致性backup并记录恢复位置。
...
6. 只有manifest、v16 schema、stable sentinel目录权限都通过后才配置/启动automatic worker。
...
- 普通recovery只允许：`harbor deploy-worker recover <job-id> --target <target-id> --confirm <job-id>`。管理员必须恢复与job冻结fingerprint一致的target配置；命令结束会重新打开v16 DB并重读stable sentinel，双闸未清或job不是`failed + rollbackComplete=true`即非零退出。
...
`DeploymentWorker.runOnce/recover` 返回最终持久化 job、DB gate与stable sentinel snapshot，不再返回“执行过”。CLI recovery完成后重新打开v16 DB并重读host sentinel；只有 `job.status=failed && rollbackComplete=true && DB gate=null && sentinel=null` 才打印成功/exit 0，否则抛错并保持needs recovery。
