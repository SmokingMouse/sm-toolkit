...
Harbor P6.1 identity normalization、Device/Agent 加载性能与 Agent roster 收敛均已部署到 Mac mini；Mew Automation normalization 已在独立 worktree 完成 schema v25 migration、全量回归与 production build，发布预检同时修复 deploy-worker 跨主机休眠失去 idle wakeup 与 macOS zombie process-group 误判，待新 exact revision 部署。生产当前仍运行 schema v24，first-owner Passkey、10 个 recovery-code hash 与 browser Session 均已持久化，system token 数据面兼容闸仍保留且 daemon credential 未改。P6.1 技术验收已完成；recovery codes 是否已由用户离线保存只能由用户本人确认，确认后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
### 2026-07-19 — Mew Automation Output/Trigger normalization（待部署）
...
- **Verified**：Automation 变更与两项发布基础设施修复合计全量 **430 tests / 2188 assertions**、root/Web typecheck、root production build（含 Harbor 与 13-page Next export）及 `git diff --check` 全绿。
- **Deployment finding / fixes**：首个 exact Job `depjob_3dg7v86oa7` 已通过 remote fetch、fresh frozen install 与 production build，但在任何 backup/cutover 前因 executor host-heavy test 沿用 Bun 5s 默认 timeout 而安全失败；该 case 已显式使用 30s budget，内部每个真实 host action 仍保留各自短 deadline。第二个 exact Job `depjob_288t5irpzt` 再次复现长期常驻 worker 在 Mac sleep 后不领取新 queue，受控 kickstart 后立即领取；其全量测试又暴露 macOS 已终止 zombie process group 偶发被 `/bin/kill` 报 EPERM。worker 现改为 launchd `StartInterval=3` 的 one-shot queue drainer，空队列正常退出；process-group probe 只忽略无法执行/持有 pipe 的全 zombie group，任一 live/unowned member 仍 fail-closed。两次失败均在 backup/cutover 前，生产保持 schema v24、gate=0、rollback_complete=true、database_backup_created=false。
- **Next**：提交并推送新 exact revision，经 local-launchd deployment worker 将生产 v24→v25；cutover 后重装 worker LaunchAgent definition，再验证自动 idle wakeup、backup、archive、integrity/FK、三项 service、public UI 的 Run/Chat/Issue 与 Schedule/Codebase 表单。daemon credential 保持不变。
