...
- **生产控制面现在唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-19 实测）：Mac mini `127.0.0.1:17777` 运行 exact `0d2d425` 的 server + daemon，SQLite 位于 `/Users/smokingmouse/.harbor/control-plane/harbor.db`，独立 deploy-worker 消费同库；MacBook 的 `127.0.0.1:17777` 只是 SSH `-L` 到 Mac mini，本机 server/worker 均 unloaded。cutover 前后 DB SHA、integrity/FK、v21→v22 migration、release manifest/fingerprint 与三 launchd PID 均验证；旧 Mac mini 7777 server、17778 reverse tunnel 和四个 orphan daemon 已停止。两台 Device 同时在线：MacBook Claude 2.1.183/Codex 0.144.5，Mac mini Claude 2.1.207/Codex 0.142.2。Bun 1.1.36 的远端 TLS issuer 问题通过 0600 System Keychain CA + target `NODE_EXTRA_CA_CERTS` 修复并实测下载，未禁用 TLS；worker health token 由 0700 stable entry 从 0600 YAML 读入进程内存，不写 plist/log。
