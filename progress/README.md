...
Harbor P6.1 identity normalization 与 Device/Agent 加载性能修复已部署到 Mac mini：生产运行 schema v24，first-owner Passkey、10 个 recovery-code hash 与 browser Session 均已持久化，system token 数据面兼容闸仍保留且 daemon credential 未改。`/api/devices` 使用持久化轻量 projection，不再把 13MB runtime Skill bundle 送往浏览器或在每次列表请求中解析。P6.1 技术验收已完成；recovery codes 是否已由用户离线保存只能由用户本人确认，确认后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
- **deploy-worker idle wakeup 未查清**（2026-07-19）：本 session 的首个性能 Job 在长时间 idle 后 15 秒仍 queued，仅重启 `com.smokingmouse.harbor.deploy-worker` 后立即领取；同一 worker 随后的四个 Job 均自动领取。当前证据更符合 idle wait 没有被新队列写入唤醒，尚未用独立回归证伪；不影响已成功的 v24 deployment，但不能把手工 kickstart 当正常流程。
