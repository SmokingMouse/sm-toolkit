...
Harbor P6.1 identity normalization、Device/Agent 加载性能、Agent roster、Mew Automation schema v25 与 deploy-worker durable wakeup 全部部署到 Mac mini。生产运行 exact `dd85fac15c78d4c4be2b04c47b117d81525b3726` / schema v25，first-owner Passkey、10 个 recovery-code hash 与 browser Session 均已持久化，system token 数据面兼容闸仍保留且 daemon credential 未改。本阶段 migration/deployment 无遗留；recovery codes 是否已由用户离线保存只能由用户本人确认，确认后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
- [ ] **Harbor（自托管多账户、多设备 Agent 调度平台，Mew 复刻）** — 主方案 `progress/harbor.md`，账户扩展方案 `progress/harbor-account-system.md`。P6.1 schema v23 Account/Auth 与 first-owner Passkey 已落地，仅 recovery-code 明文离线保管待用户确认；独立性能与 Automation migrations 占用 v24/v25，P6.2–P6.4 顺延到 v26–v28。Device enrollment/grant、Agent ACL、Automation ServicePrincipal 与 contract 尚未开始，daemon credential 仍保持 legacy compatibility。既有 P5 外部/时间性验证继续保留：真 SCM credential + webhook、真飞书群、automation 7 天与真实负载一周

## Verified Facts

- **生产控制面现在唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-19 实测）：Mac mini `127.0.0.1:17777` 运行 exact `dd85fac15c78d4c4be2b04c47b117d81525b3726` 的 server + daemon，SQLite `/Users/smokingmouse/.harbor/control-plane/harbor.db` 已从 v22 原子迁移到 v25；`ws_personal` 为 `acc_bootstrap` 创建的 personal Workspace，原 `member_system_ws_personal` ID/owner 引用保持不变。current manifest/revision/fingerprint、DB integrity/FK、135 个 maintenance triggers、maintenance gate=0、active deployment job=0 与三项 launchd service 均验证。MacBook 与 Mac mini 两台 Device 在线；daemon credential/WS hello 未改。生产已有 Account=1、active Passkey=1、unused recovery codes=10、active Session=1，first-owner ceremony 不再处于 waiting 状态；是否离线保存过明文 recovery codes 无法从仅存 hash 的 server 反推。
