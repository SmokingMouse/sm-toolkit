...
- **deploy-worker idle wakeup 未查清**（2026-07-19，两次生产复现）：性能 Job 与后续 Agent roster Job `depjob_37cgzon6x9` 都在 worker 长时间 idle 后保持 `queued / attempt 0`，后者观察 30 秒仍未领取；`launchctl kickstart -k` 同一 worker 后立即领取并由原 immutable Job attempt 1 成功。当前证据更符合 idle wait 没有被新队列写入唤醒，尚未用独立回归证伪；不影响已成功 deployment，但不能把手工 kickstart 当正常流程。
...
- **Verified**：全量 **426 tests / 2172 assertions**、root/Web typecheck、root production build（含 Harbor 与 13-page Next export）及 `git diff --check` 全绿。
- **Deployment finding / fix**：首个 exact Job `depjob_3dg7v86oa7` 已通过 remote fetch、fresh frozen install 与 production build，但在任何 backup/cutover 前因 executor host-heavy test 沿用 Bun 5s 默认 timeout 而安全失败；同一 immutable release 手工重跑定位 source/dist 两份相同 case 分别耗时 6.36s/6.55s，其余 424 tests 通过。该 case 现显式使用 15s timeout；本地实测 10.47s 通过，运行时逻辑未改。失败后生产仍为 schema v24、gate=0、rollback_complete=true、database_backup_created=false。
- **Next**：推送 exact revision，经 local-launchd deployment worker 将生产 v24→v25；验证 backup、archive、integrity/FK、三项 service、public UI 的 Run/Chat/Issue 与 Schedule/Codebase 表单。daemon credential 保持不变。
