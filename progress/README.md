...
- **Dogfood finding / fix 5**：上述临时 Chat 暴露 worktree isolation 没有 Issue 终态可触发收尾。新增 `POST /api/conversations/:id/worktree/cleanup`：仅 admin、仅 Chat、active Run 时拒绝、Device 不可达时 fail loudly；Conversation/Run 历史与 branch 保留，daemon 实际删除回报前不清 binding。定向 **6 tests / 67 assertions**、全量 **375 tests / 1803 assertions** 与 root typecheck 通过。
