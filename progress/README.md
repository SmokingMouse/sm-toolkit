...
Harbor P6.1 identity normalization、Device/Agent 性能与 roster、Mew Automation、Agent-driven self-deploy 及 v28 Codex sandbox direct-network capability 已部署。Feature Builder 是唯一开启 direct network 的生产 Agent，真实 Harbor Run 访问 GitHub 返回 HTTP/2 200；Reviewer 与 Release Builder 保持断网，self-deploy coordination 仍强制走 isolated outbox + daemon sidecar。first-owner Passkey、10 个 recovery-code hash 与 browser Session 已持久化，system token 数据面兼容闸和 legacy daemon credential 仍保留；下一阶段是 schema v29 的 P6.2 Device ownership/enrollment。
...
- [ ] **Harbor（自托管多账户、多设备 Agent 调度平台，Mew 复刻）** — 主方案 `progress/harbor.md`，账户扩展方案 `progress/harbor-account-system.md`。P6.1 schema v23 Account/Auth 与 first-owner Passkey 已落地，仅 recovery-code 明文离线保管待用户确认；v24–v28 已由性能 projection、Automation、Agent self-deploy 与 Codex network capability 使用，P6.2–P6.4 顺延到 v29–v31。Device enrollment/grant、Agent ACL、Automation ServicePrincipal 与 contract 尚未开始，daemon credential 仍保持 legacy compatibility。既有 P5 外部/时间性验证继续保留：真 SCM credential + webhook、真飞书群、automation 7 天与真实负载一周
