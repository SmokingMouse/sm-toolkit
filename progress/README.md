...
Harbor P6.1 identity normalization、Device/Agent 性能与 roster、Mew Automation、Agent-driven self-deploy、Codex direct network，以及 v29–v32 GitHub App / Run principal / first-class Repository provider 已部署。人工 Run 以发起人的 GitHub authorization 执行，Automation/self-deploy 使用 ServicePrincipal + installation token；GitHub connection 与 Repository provider 由同一生命周期维护。生产 server/daemon 当前 exact revision `5dd4fee98c0921d0d30a76b739e777be41ac5ffd`、schema v32、health healthy；system token 数据面兼容闸与 legacy daemon credential 仍保留，下一阶段是 v33+ P6.2 Device ownership/enrollment。
