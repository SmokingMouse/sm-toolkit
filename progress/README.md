...
Harbor P6.1 identity normalization、Device/Agent 性能与 roster、Mew Automation、Agent-driven self-deploy、Codex direct network，以及 v29–v32 GitHub App / Run principal / first-class Repository provider 已部署。人工 Run 以发起人的 GitHub authorization 执行，Automation/self-deploy 使用 ServicePrincipal + installation token；GitHub connection、Repository provider 与受控 push transport 已统一并 rollout 到控制面和实际执行 Device。剩余验收是用户从 Account Session 重跑原 Issue，确认真实 PR/Delivery；system token 数据面兼容闸与 legacy daemon credential 仍保留，下一阶段是 v33+ P6.2 Device ownership/enrollment。
...
- [ ] **Harbor（自托管多账户、多设备 Agent 调度平台，Mew 复刻）** — 主方案 `progress/harbor.md`，账户扩展方案 `progress/harbor-account-system.md`。P6.1 schema v23 Account/Auth 与 first-owner Passkey 已落地；v24–v28 用于性能 projection、Automation、Agent self-deploy 与 Codex network，v29–v32 已完成 GitHub App、Account/ServicePrincipal credential、v31 repair 与 first-class GitHub Repository provider。P6.2 Device ownership/enrollment/grant、Agent ACL contract 与 daemon credential migration 顺延到 v33+；既有 P5 时间性验收继续保留：真飞书群、automation 7 天与真实负载一周

## Verified Facts

- **GitHub identity、Run principal、Repository scope 已解耦，受控 push transport 已部署待最终 Account Run 验收**（2026-07-21 实测）：v29 GitHub App installation/mapping 不落 user/installation token 到 SQLite；v30 每个 Run 冻结 Account/ServicePrincipal/system principal，人工 Run 使用 Account user authorization，Automation/self-deploy 使用 installation token；v31 修复 maintenance gate 下 Automation ServicePrincipal backfill；v32 把 `github` 提升为一等 Repository provider，由 active installation + Workspace binding + Repository connection 生命周期自动维护。生产有 1 active Account authorization、1 active installation/binding、2 active `smokingmouse/sm-toolkit` aliases、62 removed mappings；两个 active alias 的 REST 视图均为 `scmProvider=github` + active connection。PR #12 feature revision `df81c878` 修复 daemon clean bare transport 缺 `HEAD`；GitHub App redelivery → Release Run `r_2i9etjonqh` → sidecar Job `depjob_26h4mulgof` generation 14 attempt 1 healthy。控制面 schema=32、integrity=ok、FK=0、gate=0，Mac mini server/daemon 与 Feature Builder 的 MacBook daemon 均完成该 feature rollout且 Device online，daemon credential 未改。实现细节见 `progress/blocks/harbor-github-app.md`、`progress/blocks/harbor-account-github-principals.md`、`progress/blocks/harbor-github-repository-provider.md`。
- **生产控制面唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-20 实测）：v28 feature revision `fcc8185575a80f37512ae5f8c98a46da67c20357` 经 Release Run `r_3ota11tnil`、sidecar Job `depjob_10dvwaarbs` attempt 1 exact cutover 并到 `healthy`；SQLite `/Users/smokingmouse/.harbor/control-plane/harbor.db` integrity=ok、FK=0、self-deploy gate=0。Mac mini server/daemon 与执行 Feature Builder 的 MacBook daemon 均完成该 revision rollout，两台 Device 在线；daemon credential/WS hello 未改。`ws_personal` 的既有 Account/Membership 引用保持不变，生产已有 Account=1、active Passkey=1、unused recovery codes=10、active Session=1；是否离线保存过明文 recovery codes 无法从仅存 hash 的 server 反推。
- **Device/Agent 列表慢的根因已经从网络到存储读模型闭环修复**（2026-07-19 生产实测）：旧 `/api/devices` 把 `installedSkills[].files[].content` 原样返回，payload 13,550,524 bytes；即使先剥离响应正文，`listDevices()` 仍每次从 SQLite 读取并解析约 13MB capabilities，阻塞并发 Agent 请求。现在 v24 `devices.capabilities_summary` 在 migration 中一次回填、每次 hello 与完整 snapshot 同 statement 更新；REST/CLI/Web 只读 `DeviceSummary`，Repository name 单列查询。生产完整 snapshot 合计 13,481,701 bytes、summary 143,151 bytes，API payload 207,353 bytes且无 `instruction/files`；Mac mini loopback Devices 249–272ms → 约20ms，公网三次中位数 Devices 405ms、Agents 126ms、Repositories 121ms。完整 snapshot 与 Skill import 保留。
- **Agent roster 按“用途 × 交付阶段”收敛，不再用相近角色名表达隐含差异**（2026-07-19 生产实测）：Agent name 可在 Workspace 内唯一校验后原地更新，Agent ID、历史 Run/Conversation 与 Automation 引用不变。active roster 固定为 `Harbor Guide`、`Harbor Feature Builder`、`Harbor PR Reviewer`、`Harbor Release Builder`、`Harbor Release Reviewer`；日常开发线绑定 `sm-toolkit`，生产发布线绑定独立 `harbor-self-hosting` checkout，Release Reviewer 在 Mac mini 跨设备运行。零使用的 Developer/Orchestrator/Triage 与同机重复 Self-hosting Reviewer 已软归档；Guide 的旧离线 queued Run 已取消并迁到在线 Mac。旧 `Auto review and merge` 已由 v25 migration 归档，历史 log 保留。
- **Automation 产品模型已从底层调度参数收敛为 Mew 的二元选择**（2026-07-19，432 tests / 2192 assertions + 双 typecheck + production build）：每条 Automation 恰好一个 Output（Run/Chat/Issue）与一个 Trigger（Schedule/Codebase）；purpose 与单飞并发由 control plane 推导，不再暴露 append/source/overlap/notify/内部 event/generic webhook。Codebase Trigger 显式选择 Agent 可访问的 Repository + 单一 SCM event，SCM webhook/refresh/CLI 经统一 projection 后投递。v25 migration 保留可无歧义映射的旧 schedule/Codebase 规则，其余写入 `automation_legacy_archive_v25`，历史 Run 不改写。生产 active Automation=0，旧 `Auto review and merge` 因不可无歧义映射已归档，28 条 automation log 保留。完整边界见 ADR `progress/decisions/2026-07-19-harbor-mew-automation-model.md`。
- **Harbor 编排层只定义机制，不定义组织结构**（2026-07-19）：Automation/人工 Run 可运行 `coordination` purpose，并通过短期 Run token 查询安全上下文、显式指定 Agent 派生同 Workspace/Conversation 的 child Run。lineage、dispatch key、深度上限、Repository/Conversation/Delivery policy 由 control plane 校验，但 Agent 选择完全不由 Harbor 推断。持久 Domain Event 继续作为内部审计/状态事实，不再是用户 Automation Trigger。跨 Device Review 只接受 Provider 证明的 exact head revision，并在 daemon 上 detached checkout；缺可信 revision 时只允许原 worktree 本地审查。开放编排边界见 ADR `progress/decisions/2026-07-19-harbor-open-agent-orchestration.md`。
...
- **Codex direct network 是显式 Agent/Run capability，不是 auto-edit 或 full access 的别名**（2026-07-20 本机实测）：v28 `sandboxNetworkAccess` 旧值默认 false，dispatch 冻结到 RunSpec；仅 workspace-write 初次/resume 显式传 `sandbox_workspace_write.network_access=true|false`，readonly 不为联网切换成可写，full 不生成伪约束。Codex 0.144.5 真实 HTTPS probe 到 GitHub 返回 HTTP/2 200；merged self-deploy coordination 即使 Agent 开网也强制 false，继续走 isolated outbox + daemon sidecar。广域 egress 风险与边界见 ADR `progress/decisions/2026-07-20-harbor-codex-sandbox-network.md`。
...
- **Harbor Agent 的 Skill 边界由 control plane 决定，不继承 Device 环境**（2026-07-19 参数测试 + Codex prompt debugger + self-hosting Review 实测）：scheduler 仍只注入 Agent instruction + 当前绑定的 Harbor Skill 快照；daemon 对 Claude 强制 `--safe-mode --disable-slash-commands`，对 Codex 忽略用户 config/rules、关闭 plugins/自动 Skill catalog，并把用户、bundled、admin、checkout 祖先 Skill 名逐一禁用，显式 `$skill` 也无法注入。Agent environment 在 server/daemon 双层禁止改写 `HOME/CODEX_HOME/CLAUDE_CONFIG_DIR/PATH`；Skill 扫描不完整、文件超限或名称超过 CLI 安全上限时整个 Run fail-closed，不返回截断 denylist。通用 `@sm/agent` 默认行为不变。Codex 不使用会删除共享 system Skill cache 的 `skills.bundled.enabled=false`。完整决策见 `progress/decisions/2026-07-19-harbor-agent-skill-runtime-isolation.md`。
...
- **Harbor Prompt 配置是 Workspace 级两段式 pipeline**（2026-07-19 实测）：Issue / Chat 在 dispatch 时组合稳定 `session context` 与本次 `event trigger`；Automation 使用 schedule/manual/Codebase event。Run 持久化 `promptEvent` 与 `triggerRef`，不从可变 Conversation 事后猜触发来源；旧 Harbor-event block 仅为历史 Run 渲染保留且不再出现在 Settings。
- **Repository 的唯一产品配置源是 Agent，Codebase Trigger 只做受限选择**（2026-07-19 实测）：Workspace 不配置统一仓库地址；每个 Agent 必须绑定至少一个 Repository 且目标 Device 有 mount。Issue / Chat / AI draft / Schedule Automation 继承 Agent primary Repository；Codebase Trigger 只能从该 Agent 可访问的 Codebase Repository 中选一个。Run 冻结 repository / mount / execution root，Review Agent 必须绑定实现仓库。
...
### 2026-07-21 — GitHub Repository provider v32（已部署）
- **Root cause**：GitHub identity/installation/Repository connection 已可用，但 `Repository.scmProvider` 仍只有 `local | codebase`，Settings 也只显示两者；连接成功的 GitHub Repository 因而仍显示 local，Agent 为继续交付会误改为 Codebase，再因不存在 Codebase MR number 失败。
- **Decision / Done**：provider 统一为 `local | github | codebase`；`github` 只能由 active GitHub connection 产生并随 disconnect/suspend/delete/removal 回到 local，active connection 与 Codebase 互斥。Run context、REST/UI、credential broker、Delivery 与 builtin Harbor Skill 使用同一状态；implementation Agent 的 provider 必须精确匹配 Repository，禁止 manual/provider 绕过；daemon credential 409 只透传 500 字内清洗后的 Harbor error，不暴露 secret。
- **Verified**：全量 475 tests / 2431 assertions、focused source+dist 30 tests / 196 assertions、root typecheck、全部 workspace production build、`git diff --check` 全绿。生产 v31 online backup 本地副本迁移后 schema=32、integrity=ok、FK=0，2 active aliases→github、62 unmapped→local。
- **Deployed**：PR #10 merge `5dd4fee98c0921d0d30a76b739e777be41ac5ffd`；GitHub merge event 自动触发 Release Run `r_3aqwcnqraz`，sidecar Job `depjob_2alfeix0cj` generation 12 attempt 1 fresh test/build/cutover/health 全部 succeeded。生产 REST 返回两个 sm-toolkit alias 均为 `github` + `smokingmouse/sm-toolkit active`，schema=32、health exact revision、maintenance=false，server/daemon 同 revision，daemon credential 未改。
- **Next**：刷新 Settings 即可看到 GitHub。原 Issue `c_1d0ymfs03b` 需要用户从已登录 Session 再 Continue 一次，验证人工 Account principal 的 controlled push + PR/Delivery；P6.2 Device ownership/enrollment/grant 与 daemon credential migration 进入 v33+。

### 2026-07-21 — Account controlled push transport 修复（已部署，待最终重跑）
- **Root cause**：Account principal、GitHub connection 和 credential broker 均已通过；Run `r_47hshysn17` 失败是 daemon 构造的 clean bare transport 缺 Git repository discovery 必需的 `HEAD`，所以 push 在认证前报 `not a git repository`。Issue 安全退回 Ready，未创建虚假 PR/Delivery。
- **Done**：transport 写入指向固定 source ref 的 `HEAD`；测试让 Git 真正识别该 bare repository、向本地 bare remote push 并核对目标 branch SHA，不再只检查文件结构。PR #12 feature merge `df81c878`。
- **Verified / deployed**：全量 475 tests / 2441 assertions、Harbor build、diff check 全绿；App delivery `3832481130917920768` redelivery 202 → Release Run `r_2i9etjonqh` → sidecar Job `depjob_26h4mulgof` generation 14 attempt 1 healthy。控制面 health exact feature revision、schema=32、integrity=ok、FK=0、gate=0；MacBook immutable release frozen install + regression 后 daemon PID 稳定、Device online。首次 MacBook 启动因 prep cwd 错误缺依赖，已在 online 验收中捕获并修正；旧 plist/release 可回滚，credential 未改。
- **Next**：用户从已登录 Account Session 对 `c_1d0ymfs03b` 点 Continue；验收 branch push、GitHub PR 与 Delivery 后再关闭本 block。system token 不能替代这个动作。

### 2026-07-20 — Codex sandbox direct-network capability（已部署）
- **Decision**：用户明确要求 Harbor Codex Agent 能访问 GitHub/外部网络；增加 Agent 显式 `sandboxNetworkAccess`，不全局开网、不切 full。只对 workspace-write 生效；read-only 继续离线；self-deploy 永远覆盖为 false 并保留 sidecar。ADR：`progress/decisions/2026-07-20-harbor-codex-sandbox-network.md`。
- **Done**：SQLite v28、Store/REST/CLI/Web、scheduler RunSpec、daemon 与 `@sm/agent` Codex initial/resume 参数链路已完成并由 PR #1 merge；Release Run `r_3ota11tnil` 自动提交 exact revision，sidecar Job `depjob_10dvwaarbs` attempt 1 部署到 healthy。生产只给 `Harbor Feature Builder` 开启 network；`Harbor PR Reviewer` / `Harbor Release Builder` 保持关闭。执行 Agent 的 MacBook Device daemon 同步滚到 exact `fcc8185`，两个验收 Chat worktree 均已清理。
- **Verified**：Codex CLI 0.144.5 真实 workspace-write HTTPS probe 到 GitHub=HTTP/2 200；v27 fixture 与生产在线 backup migration drill 均证明 v28 保留 12 Agents/3 条 self-deploy job 历史、默认关闭、integrity=ok、FK=0。生产首次 Run `r_2hxh4hjh64` 在旧 MacBook daemon 上仍 DNS blocked，证伪“只升级 control plane 即完成”；Device rollout 后 Run `r_3lfwjxskdb` 返回 `HTTP/2 200`。最终 schema=28、integrity=ok、FK=0、gate=0；全量 **421 tests / 2143 assertions**、root/Web typecheck、全部 workspace production build（Next 13 static pages）与 `git diff --check` 通过。
- **Deployment note**：PR merge 的首次 GitHub delivery 在公网入口等待满 10 秒后 timeout=500，Harbor 未落对应 Run；确认 DB 可写、无重复 delivery 后 redelivery 返回 202 并正常触发。协议新增 RunSpec 字段时，实际承载目标 Agent 的 Device daemon 也属于 rollout 必检项。
- **Next**：本阶段 migration/deployment 无遗留；进入 schema v29 的 P6.2 Device ownership/enrollment，届时再迁 daemon credential。
...
- **Verified**：Automation 变更与发布基础设施修复合计全量 **432 tests / 2192 assertions**、root/Web typecheck、root production build（含 Harbor 与 13-page Next export）及 `git diff --check` 全绿。
- **Deployment finding / fixes**：首个 exact Job `depjob_3dg7v86oa7` 已通过 remote fetch、fresh frozen install 与 production build，但在任何 backup/cutover 前因 executor host-heavy test 沿用 Bun 5s 默认 timeout 而安全失败；该 case 已显式使用 30s budget，内部每个真实 host action 仍保留各自短 deadline。第二个 exact Job `depjob_288t5irpzt` 再次复现长期常驻 worker 在 Mac sleep 后不领取新 queue，受控 kickstart 后立即领取；其全量测试又暴露 macOS 已终止 zombie process group 偶发被 `/bin/kill` 报 EPERM。worker 现改为 launchd `StartInterval=3` 的 one-shot queue drainer，空队列正常退出；process-group probe 只忽略无法执行/持有 pipe 的全 zombie group，任一 live/unowned member 仍 fail-closed。setup 会保留当前 uid/0700/non-symlink 的 host credential wrapper，避免 SSH bootstrap 把 GUI LaunchAgent 改成拿不到 health secret 的 Bun 直启。前两次失败均在 backup/cutover 前，生产当时保持 schema v24、gate=0、rollback_complete=true、database_backup_created=false。
- **Deployed**：Release Issue `c_3vtj8a0yhn` / Delivery `del_3nnn7qo3p3` / Job `depjob_2q49woetce` 将 `9bfda32a680a0f671e6c6c21f2fb679300086daa` 一次成功 cut over 到 schema v25；Release Issue `c_2wyyh7eux9` / Delivery `del_d5dv3ey6n1` / Job `depjob_2lkqba4b3n` 又将 wrapper preservation exact `dd85fac15c78d4c4be2b04c47b117d81525b3726` 一次成功上线。最终 Job 在 enqueue 5 秒内无需 kickstart 自动领取，fresh build/test、DB backup、cutover 与 exact health 均 succeeded；新 setup 实测保留 `/bin/zsh` + 0700 wrapper，launchd `StartInterval=3` 连续 `runs=76`、last exit=0。
- **Production acceptance**：schema=25、integrity=ok、FK=0、135 maintenance triggers、gate/job=0；旧 Automation archive=1、active=0、历史 log=28，Account=1/Passkey=1/unused recovery=10/Session=1。public/local health 均返回 final exact revision；公网三次中位数 Devices 442ms、Agents 124ms、Repositories 118ms。disabled Schedule/Chat production smoke 创建与清理成功，legacy Purpose=400；Codebase/Issue 因生产尚无启用 Codebase 的 Repository 正确 fail-closed。daemon credential/WS hello 未改。
- **Next**：本阶段 migration/deployment 无遗留。进入 P6.2 前只需用户确认 recovery-code 明文已离线保存；Codebase 真 credential/webhook、真飞书与 7 天/一周时间性 dogfood 继续作为既有外部验收。

### 2026-07-19 — Agent roster normalization + rename deployment
- **Decision**：保留两条职责明确的执行线，而不是把 Builder/Developer/Self-hosting 当并列角色。日常开发线为 Feature Builder + PR Reviewer；生产发布线为 Release Builder + 跨设备 Release Reviewer；Guide 只做只读问答。Release 线存在的理由是独立 checkout、exact revision 与 deployment/fencing/rollback 复核，不是第二套日常开发。
- **Done**：`PATCH /api/agents/:id` 新增 Workspace 内唯一、trim/空值校验的原地 rename，Store 与 Agents 配置页同步支持编辑 Name；保留 5 个 Agent 并更新职责描述/Reviewer 路由，软归档 4 个零使用或重复 Agent。取消离线 `harbor-preview` 上的旧 queued Chat Run，将 Guide 迁到在线 `SmokingMouse-Mac`；Automation 继续绑定同一 Reviewer ID。
- **Verified**：本地定向 **8 tests / 56 assertions**、全量 **221 tests / 1117 assertions**、root typecheck、Next production build 13 pages 与 `git diff --check` 全绿；self-hosting Run 在 clean worktree 复核 exact SHA、定向测试和双 typecheck。生产 `/api/health` 返回 exact revision、maintenance=false；schema=24、integrity=ok、FK=0、maintenance gate=0、active Job=0，server/daemon/worker 均 running；Agents API 仅返回 5 个目标 Agent，Automation 展示新 Reviewer 名称。
- **Deployed**：提交 `860847decd460371cf80ae35af2d5ef97f09fa75` 经 Issue `c_1vqw6yfcic` / Delivery `del_i9c8aivh59` / Job `depjob_37cgzon6x9` exact-revision 上线。Job 首先再次复现 idle wakeup，受控 kickstart worker 后原 generation attempt 1 完成 fresh install、全量 build/test、backup、cutover 与 exact health，最终 succeeded/healthy。
- **Next**：Agent roster 无遗留迁移项。独立修复 deploy-worker idle wakeup；产品主线仍是确认 recovery-code 明文已离线保存后进入 P6.2 Device ownership/enrollment（现顺延至 schema v26，届时才迁 daemon credential）。

### 2026-07-19 — Device/Agent list performance + production v24
- **Root cause / fix**：`/api/devices` 只剥离 Skill `instruction`，遗漏 `files[].content`；Repository view 为取 Device name 也 hydrate 完整 capabilities。第一层新增 `DeviceSummary`、移除正文、增加 `fileCount` 并改为 name-only SQL。生产首轮验收又证伪“payload 小就够了”：每次列表仍解析 13MB DB blob并阻塞 Agent 并发请求，因此第二层新增 v24 持久化 `capabilities_summary`，migration maintenance transaction 内 backfill，hello 原子双写，旧 schema fallback 保留。
- **Verified**：root typecheck、全部 workspace production build、Next 13 static pages、全量 **422 tests / 2168 assertions**；独立 Harbor Run 再次验证 exact revision 与全量测试。生产 schema=24、integrity=ok、FK=0、gate/job=0、132 triggers；两台真实 Device 在线，projection 不含 `instruction/files`，完整 Skill snapshot/import 与 daemon credential/hello 保持不变。
- **Deployed**：第一层 `3ad4d3e1dccfeba6da9900c0b8ae062f84ca5192` 经 Issue `c_33wrbmsp3t` / Delivery `del_ekqo6vprcw` 上线；首次 stop-proof fail-closed 后走受支持 recovery + Retry `depjob_3w3vh4n3vh` 成功。最终 v24 `5d084332c0b7de8f792e70f39e27f11697c5ee0f` 经 Issue `c_1fzfdeq2nl` / Delivery `del_z3elbfz8xd` 上线；首个 pre-cutover test attempt 瞬时失败，同一 immutable release 重跑 422/422 后支持路径 Retry Job `depjob_1if20sfmg6` succeeded/healthy。
- **Next**：P6.2 Device ownership/enrollment 因 v24/v25 已由性能 projection 与 Automation normalization 使用而顺延到 schema v26；届时才迁移 daemon credential。独立修复 deploy-worker idle wakeup；recovery codes 明文是否已离线保存由用户确认。

### 2026-07-19 — P6.1 identity normalization + production v23
- **Done**：在独立 worktree 完成 v22 canonical fixtures、只读 `harbor db identity-report`、schema v23 Account/AuthIdentity/Passkey/Recovery/Session/PAT/Invitation、Account principal/RBAC、first-owner/invitation/recovery/login ceremonies、Workspace switcher 与 Account/Workspace settings；系统 token compatibility gate 保留，daemon credential 与 WS hello 无 diff。提交 `1abb83d`；另将 Bun lock 从 Mac mini 不可达的内部 registry 重锁到 public npm，并用 repo-local `bunfig.toml` 固定，提交 `4a08789`。
- **Verified**：生产 v22 在线 backup 的 report 为 `1 legacy member → 1 Account / 1 Membership`、无 blocker/warning且 SHA 不变；副本 migration drill 与真实 cutover 后均为 schema 23、Account=1、Membership=1、integrity=ok、FK=0。源码+dist **414 tests / 2136 assertions**、root typecheck/build、Web 13 static pages 与 public `/login` 全绿；两台真实 Device heartbeat 在线。
- **Deployed**：`main@4a0878905e7290232745f74eceebb8e0fd2aba03` 经 Harbor Issue `c_1rqznin12z` / Delivery `del_1jatzxy50i` exact-revision 部署。首次 attempt 在 fresh install 前暴露 lockfile 内部 registry 并安全失败；修复后首次 cutover 又因 launchd stop proof fail-closed 到 `needs_recovery`，显式 `deploy-worker recover` 证明旧 baseline 后清闸，Retry Job `depjob_nkfdb6bmm1` succeeded/healthy。生产 v22 rollback DB `/Users/smokingmouse/.harbor/deploy-state/depjob_nkfdb6bmm1/attempt-1/database.sqlite` 为 0600、integrity=ok、FK=0。
- **Next**：用户访问 `https://harbor.home.smokingmouse.cn/login`，输入 system bootstrap token，用 Touch ID 创建 first-owner Passkey并保存 10 个只展示一次的 recovery codes；随后复核 `bootstrap.required=false`、Passkey=1、RecoveryCode=10，再进入 P6.2。部署 worker 的 idle wakeup 另立根因修复，不与 daemon credential 迁移混做。
...
- **Done**：完整实施方案落 `progress/harbor-account-system.md`，覆盖实体基数/生命周期、Passkey/Session/PAT、Device enrollment/credential/grant、Agent 可见性矩阵、Automation/Run principal、目标 schema、API/UI、现顺延至 v23–v28 的 migration、回滚/观测、威胁模型与生产验收；同步更新 glossary 与 Harbor 总方案，`.gitignore` 忽略 macOS `.DS_Store`。
...
- **Done**：SQLite v22 新增 durable Domain Event、Run lineage/dispatch key/exact Review checkout 与 `coordination` purpose；Automation/Trigger 可编辑并支持 `issue.created / issue.ready`；任意 action-capable Run 可读取安全上下文并显式 dispatch child Run；Provider head SHA 可在目标 Device 创建 detached per-Run checkout，重连会回收 orphan；Web 与 built-in Harbor Skill 同步开放式编排模型。
- **Verified**：源码 **204 tests / 971 assertions**、GitHub Provider **32 tests / 121 assertions**、root typecheck、Harbor Web typecheck、全部 workspace production build（Next 12 static pages）与 `git diff --check` 全绿；提交 `60759a6` 已 fast-forward 合入 `main`。
- **Deployed**：`main` 与 self-hosting branch 已推送至 `0d2d425`。Mac mini 预构建 release 经 390 tests / 1888 assertions 与 full build，通过一致性 backup 把生产 DB 从 MacBook v21 单写 cut over 到 Mac mini v22；revision-aware health、target fingerprint、manifest hash、DB integrity/FK、三项 launchd service、Web/Automations 200 与双 Device CLI/route 上报全部通过。MacBook 本机只保留 SSH local-forward + daemon。`https://harbor.home.smokingmouse.cn` 已复用通配 DNS/证书与 frp 443 链路，由 Caddy 反代到 Mac mini server；公网 root=200、无 token API=401、带 token health=当前 revision。
- **Next**：外部真 GitHub/Codebase credential + webhook、真飞书群与 7 天/一周时间性负载仍需单独验收。
...
- **Decision**：Harbor Run 默认关闭环境 Skills，`@sm/agent` 其他调用方保持兼容默认。Claude 使用官方 safe mode；Codex 关闭 catalog/plugins，并用启动时的有界名字快照禁掉显式 Skill 注入；任何环境根/CLI 身份分叉或 denylist 不完整均 fail-closed。模型清单始终来自 Agent 当前 Device，不在 server 伪造全局目录。完整隔离理由见 ADR。
- **Done**：新增跨 Runtime `environmentSkills` 契约、Codex 环境 Skill 扫描与初次/resume 参数隔离；Agent 编辑页复用 Device 的 Claude aliases/sm-toolkit routes 和 Codex model cache，保留 unknown current override。首次 self-hosting Reviewer 对 merge commit 返回 REQUEST_CHANGES：Agent environment 可覆盖 `HOME/CODEX_HOME`，且 512 名上限会静默截断；修复在 API 与 daemon spawn 双层保留 Runtime 环境变量，并让 scan error/oversize/overflow 直接拒绝 Run。
- **Verified**：修复后定向 **18 tests / 78 assertions**、与 production lineage 收敛后全量 **386 tests / 1850 assertions** ✓；root typecheck、全部 workspace production build（Next 12 static pages）、`git diff --check` ✓；Codex `debug prompt-input` 实测 disabled `$agent-browser` 不注入 Skill 内容。
- **Next**：重新走独立 self-hosting Review，通过后把 exact revision 交给 Local launchd Deployment Provider；旧 Claude/Codex session 的历史 transcript 不会被回写清理，新 Run 起严格生效。
...
- **Accepted / deployed 2**：Mac mini Reviewer 在真实 Issue `c_38uewikuuy` 的第二台 Device worktree 检查 exact `2097462`，首个内部反例子任务超时后经 Harbor 正常 cancel，resume Run `r_2qlg79v6qj` 最终 APPROVE 并把 Issue 推到 Review。manual/local Delivery `del_3tct52b7f7` 以完整 revision `209746272881048a8365b9487335bde373bdeb18` 通过 review/check/merge gates，自动 job `depjob_9de98c11tn` attempt 1 完成 fresh install/build/full test/backup/cutover/health；Delivery=`succeeded`、Issue=`done`、current release=`depjob_9de98c11tn-g1-a1`、manifest exact、maintenance gate=0、active job=0、health=200、integrity=ok、双 Device heartbeat 在线。生产新 API 随后清理 Chat `c_an60xb3se8`：Chat/Run 历史保留且 status=open，binding=null，Mac mini 目录与 Git registry 均不存在，server 收到 daemon success proof。
- **Next**：配置真 SCM push/PR credential 并跑外部 webhook acceptance；随后继续真飞书、automation 7 天与真实负载一周。
