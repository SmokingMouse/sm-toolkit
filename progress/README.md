...
Harbor P6.1 identity normalization、Device/Agent 加载性能与 Agent roster 收敛均已部署到 Mac mini：生产运行 schema v24，first-owner Passkey、10 个 recovery-code hash 与 browser Session 均已持久化，system token 数据面兼容闸仍保留且 daemon credential 未改。`/api/devices` 使用持久化轻量 projection；Agent 支持保留 ID/历史的原地重命名，线上 9 个 active Agent 已收敛为 Guide、日常开发 Builder/Reviewer、生产发布 Builder/Reviewer 共 5 个。P6.1 技术验收已完成；recovery codes 是否已由用户离线保存只能由用户本人确认，确认后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
- **Harbor 编排层只定义机制，不定义组织结构**（2026-07-19，204 个源码测试 + root typecheck/build）：`issue.created / issue.ready / issue.review_ready / delivery.merge_ready / delivery.merged` 先持久化为幂等 Domain Event，再由用户配置的 Automation 消费；Automation 可直接指定目标 Agent，或运行 `coordination` Run 后通过短期 Run token 查询安全上下文、显式指定 Agent 派生同 Workspace/Conversation 的 child Run。lineage、dispatch key、深度上限、Repository/Conversation/Delivery policy 由 control plane 校验，但 Agent 选择完全不由 Harbor 推断。跨 Device Review 只接受 Provider 证明的 exact head revision，并在 daemon 上 detached checkout；缺可信 revision 时只允许原 worktree 本地审查。该决策取代“固定 Orchestrator / Developer / Reviewer 团队”的产品策略，不影响用户自行配置同名角色。完整边界见 ADR `progress/decisions/2026-07-19-harbor-open-agent-orchestration.md`。
...
- **Next**：Agent roster 无遗留迁移项。独立修复 deploy-worker idle wakeup；产品主线仍是确认 recovery-code 明文已离线保存后进入 P6.2 Device ownership/enrollment（schema v25，届时才迁 daemon credential）。
...
- **Next**：P6.2 Device ownership/enrollment 因 v24 已被性能 projection 使用而顺延到 schema v25；届时才迁移 daemon credential。独立修复 deploy-worker idle wakeup；recovery codes 明文是否已离线保存由用户确认。
