...
Harbor P6.1 identity normalization、Device/Agent 性能与 roster、Mew Automation、Agent-driven self-deploy 已部署；生产运行 exact `1336778cb6cb4d1996a26fa93ff6a03d63b9f0ba` / schema v27。当前在 `codex/harbor-agent-network-access` 完成 v28 Codex sandbox direct-network capability，本地 migration drill、全量测试、production build 与真实 Codex HTTPS probe 均通过，待提交、PR merge、sidecar 部署和生产 Run 验收。first-owner Passkey、10 个 recovery-code hash 与 browser Session 已持久化，system token 数据面兼容闸和 legacy daemon credential 仍保留；P6.2 Device ownership/enrollment 顺延到 schema v29。
...
- **生产控制面现在唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-20 实测）：Mac mini `127.0.0.1:17777` 运行 exact `1336778cb6cb4d1996a26fa93ff6a03d63b9f0ba` 的 server + daemon，SQLite `/Users/smokingmouse/.harbor/control-plane/harbor.db` 为 schema v27；`ws_personal` 为 `acc_bootstrap` 创建的 personal Workspace，原 `member_system_ws_personal` ID/owner 引用保持不变。current revision/fingerprint、DB integrity/FK、self-deploy gate=0、legacy runtime deployment tables=0 与 server/daemon/sidecar service 均已验证，Delivery 历史部署审计保留在 immutable archive。MacBook 与 Mac mini 两台 Device 在线；daemon credential/WS hello 未改。生产已有 Account=1、active Passkey=1、unused recovery codes=10、active Session=1，first-owner ceremony 不再处于 waiting 状态；是否离线保存过明文 recovery codes 无法从仅存 hash 的 server 反推。
...
### 2026-07-20 — Codex sandbox direct-network capability（进行中）
...
- **Done**：SQLite v28、Store/REST/CLI/Web、scheduler RunSpec、daemon 与 `@sm/agent` Codex initial/resume 参数链路已完成；P6.2–P6.4 schema 号顺延到 v29–v31。
- **Verified**：Codex CLI 0.144.5 真实 workspace-write HTTPS probe 到 GitHub=HTTP/2 200；生产 schema v27 在线 backup migration drill 到 v28 保留 12 Agents/3 条 self-deploy job 历史，默认 enabled=0、integrity=ok、FK=0，原生产 DB 复核仍为 v27/gate=0；全量 **421 tests / 2143 assertions**、root/Web typecheck、全部 workspace production build（Next 13 static pages）与 `git diff --check` 通过。
- **Next**：提交推送并 merge，以现有 Release Agent + self-deploy sidecar 部署 exact revision，只给目标 Codex Agent 开启网络并跑真实 Run probe。
