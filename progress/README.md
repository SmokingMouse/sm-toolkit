...
Harbor P6.1 identity normalization 已部署到 Mac mini：生产 SQLite 升至 schema v23，全局 Account、Workspace Membership、Session/PAT/Invitation、Passkey bootstrap/login 与 Workspace switcher 已上线，system token 数据面兼容闸仍保留且 daemon credential 未改。唯一未完成的人机步骤是用户在生产 HTTPS origin 用 Touch ID 绑定 first-owner Passkey并保存一次性 recovery codes；完成后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
- [ ] **Harbor（自托管多账户、多设备 Agent 调度平台，Mew 复刻）** — 主方案 `progress/harbor.md`，账户扩展方案 `progress/harbor-account-system.md`。P6.1 schema v23 与 Account/Auth 已部署，待用户完成 first-owner Passkey ceremony；P6.2–P6.5 的 Device enrollment/grant、Agent ACL、Automation ServicePrincipal 与 contract 尚未开始。既有 P5 外部/时间性验证继续保留：真 SCM credential + webhook、真飞书群、automation 7 天与真实负载一周

## Verified Facts

- **生产控制面现在唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-19 实测）：Mac mini `127.0.0.1:17777` 运行 exact `4a0878905e7290232745f74eceebb8e0fd2aba03` 的 server + daemon，SQLite `/Users/smokingmouse/.harbor/control-plane/harbor.db` 已从 v22 原子迁移到 v23；`ws_personal` 为 `acc_bootstrap` 创建的 personal Workspace，原 `member_system_ws_personal` ID/owner 引用保持不变。部署 Job `depjob_nkfdb6bmm1` succeeded/healthy，pre-cutover v22 backup 与 rollback anchor 均为 0600，current manifest revision/fingerprint/hash、DB integrity/FK、maintenance gate=0、public root/login=200、unauth API=401 全部验证。MacBook 与 Mac mini 两台 Device 同时在线；daemon credential/WS hello 未改。首次部署的 launchd stop proof 进入 fail-closed recovery，显式 recovery 验证旧 baseline 并清闸后，受支持的 Retry 成功；长驻 worker 两次未及时领取 queued Job，均需只重启 worker 才 claim，wakeup 根因尚待后续单独修复。
...
- **deploy-worker idle wakeup 未查清**（2026-07-19）：Mac mini 长驻 worker 进程存活，但两次没有及时 claim 新 queued Job；仅重启 `com.smokingmouse.harbor.deploy-worker` 后立即领取。当前假设是 idle loop/定时等待没有被新队列写入唤醒，尚未用独立回归证伪；不影响已经成功的 v23 deployment，但下一次部署前应先定位并修复，不能把手工 kickstart 当正常流程。
