...
- **Operational finding / fix 4**：worker setup 连续两次在 `bootout` 后立即 `bootstrap` 命中 launchd label namespace 的短暂 EIO，人工稍后原命令即可成功。daemon 与 worker setup 现共享只针对 exact `Bootstrap failed: 5: Input/output error` 的 2 秒有界 retry；任何其他 launchctl failure 立即原样失败，不泛化吞错。定向 service **6 tests / 18 assertions**、全量 **369 tests / 1765 assertions** 与 root typecheck 通过。
- **Next**：提交并 Review launchd setup retry 后，用真实 worker setup 验收；随后迁移 Mac mini control plane、登记同仓 checkout 并跑双 Device。
