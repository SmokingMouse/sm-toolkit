...
Harbor P4.12「Mew 式 Skill 配置」已完成：Workspace Skill 支持手动编辑/上传、本机 Runtime 同步、Agent 多选绑定，并在每次 Run 下发时真正注入 system prompt。当前回到 P5 时间性验证，等待真双机 / 真飞书 / automation 7 天 / dogfood 一周。
...
- [ ] **Harbor（个人多设备 Agent 调度平台，Mew 复刻）** — 主方案 `progress/harbor.md`。P1–P4.12 已完成（P4.8–P4.12 Mew 对标、模型路由、敏捷闭环、AI draft 与 Skill 配置见 `progress/harbor-mew-ux.md`）。仅剩 P5 时间性验证——真双机 Tailscale、真飞书群冒烟、automation 连跑 7 天、真实负载一周，全部依赖用户环境
