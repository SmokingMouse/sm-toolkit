...
Harbor P6.1 identity normalization、Device/Agent 加载性能、Agent roster 与 Mew Automation schema v25 已部署到 Mac mini；发布预检同时修复 deploy-worker 跨主机休眠失去 idle wakeup、macOS zombie process-group 误判及 SSH setup 丢失 private credential wrapper，最后一项代码补丁待 exact-revision 部署。生产当前运行 schema v25，first-owner Passkey、10 个 recovery-code hash 与 browser Session 均已持久化，system token 数据面兼容闸仍保留且 daemon credential 未改。P6.1 技术验收已完成；recovery codes 是否已由用户离线保存只能由用户本人确认，确认后进入 P6.2 Device ownership/enrollment。真 SCM credential/webhook、真飞书与时间性负载验证继续作为既有 P5 外部验收并行保留。
...
- **生产控制面现在唯一运行在 Mac mini，MacBook 是纯 Device**（2026-07-19 实测）：Mac mini `127.0.0.1:17777` 运行 exact `860847decd460371cf80ae35af2d5ef97f09fa75` 的 server + daemon，SQLite `/Users/smokingmouse/.harbor/control-plane/harbor.db` 已从 v22 原子迁移到 v24；`ws_personal` 为 `acc_bootstrap` 创建的 personal Workspace，原 `member_system_ws_personal` ID/owner 引用保持不变。current manifest/revision/fingerprint、DB integrity/FK、132 个 maintenance triggers、maintenance gate=0、active deployment job=0 与三项 launchd service 均验证。MacBook 与 Mac mini 两台 Device 在线；daemon credential/WS hello 未改。生产已有 Account=1、active Passkey=1、unused recovery codes=10、active Session=1，first-owner ceremony 不再处于 waiting 状态；是否离线保存过明文 recovery codes 无法从仅存 hash 的 server 反推。
...
### 2026-07-19 — Mew Automation Output/Trigger normalization（已部署，worker setup 补丁待部署）
...
- **Deployed**：Release Issue `c_3vtj8a0yhn` 在 clean exact worktree 完成 build、430/430 tests 与 typecheck；Delivery `del_3nnn7qo3p3` / Job `depjob_2q49woetce` 将 `9bfda32a680a0f671e6c6c21f2fb679300086daa` 一次成功 cut over 到 schema v25。重装 worker definition 后 `StartInterval=3` 已证实 `runs 2→7`，但 setup 的 Bun 直启拿不到 SSH domain health secret；生产已恢复现有 0700 wrapper 后连续 exit 0，代码补丁会自动保留该 host wrapper。
- **Next**：部署 worker wrapper 保留补丁；再验证 automatic cadence、backup、archive、integrity/FK、三项 service、public UI 的 Run/Chat/Issue 与 Schedule/Codebase 表单。daemon credential 保持不变。
