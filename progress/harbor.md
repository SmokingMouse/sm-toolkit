...
| **Conversation** | 对话容器，`kind: chat \| issue` 二态。Chat=临时探索，Issue=留档任务 | Chat 始终绑定 Agent；Issue 的当前 Assignee 可空。1 Conversation — N Run，同一 Conversation 严格串行 |
...
| **Run** | 一次 CLI 进程调用（一条 prompt → 一个 result），带 `purpose=implementation\|review\|verification` 快照 | 状态 `queued → running → succeeded / failed / canceled`。只有 implementation 续接 Assignee session 并推进 Issue；review/verification 是 Review 内的独立执行 |
...
- Reviewer 若无法访问 implementation worktree（不同 Device/workdir）会在调度层拒绝，REST/CLI/飞书不能绕过。
