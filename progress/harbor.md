...
- merge 已完成且 human approval / Harbor checks 通过后，server 幂等 enqueue SQLite durable job。job 绑定 implementation generation、exact merged revision 与 lease token；重试创建新 generation，旧 revision、旧 lease、重复 callback 均不能覆盖当前 Delivery。
- 独立 `harbor-deploy-worker` host 进程执行 exact revision 校验与 checkout、配置化 install/build/test、一致性 SQLite backup、原子 launchd definition/current release 切换、bootout/bootstrap 与 authenticated health check；它不依赖 server/daemon 常驻，队列与 checkpoint 在重启后恢复。
- health/启动失败会恢复旧 plist、release symlink 与部署前 DB backup并重启旧 release；任何 restore/health 不确定性都持久化为 Deployment failed，不推进 Issue Done。日志在进入 Delivery audit 前截断并按 target secret/path 脱敏。
- SQLite v14 从 v13 无损迁移 Delivery evidence 并新增 durable jobs；Web 展示 target、queued/running/succeeded/failed、失败原因与 Retry。无 target 时原 manual deployment fallback 完全保留。
- 测试只用 fake FS/process/launchd/HTTP/clock 和隔离 SQLite，覆盖 server restart、lease reclaim、duplicate callback、health rollback、backup restore failure、stale revision/generation、crash checkpoint 与 manual fallback；未操作本机 launchd、真实 DB 或真实 YAML。
