...
- SQLite v16 从 v13 无损迁移：manual/GitHub no-target running语义保留，只有 active automatic job进入 recovery；无可信 baseline manifest拒绝自动 cutover，旧 v14缺 anchor使用显式 legacy ack。Web 展示 target、active checkpoint/attempt/fence、有界脱敏 log、失败原因、Retry/recovery；无 target 时 manual fallback完全保留。
