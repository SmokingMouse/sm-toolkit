...
| 术语 | 定义 | 关键关系与边界 |
|---|---|---|
| **Workspace** | 面向用户的一组工作上下文，是 Harbor 一级逻辑 scope | 隔离 Agents / Skills / Conversations / Automations / prompt settings / Usage；不是 Repository、目录、Device 或租户，可零仓库或多仓库 |
| **Repository** | Workspace 内的逻辑代码资源 | 通过 RepositoryMount 映射到各 Device checkout；不是 Agent 的永久 cwd |
| **RepositoryMount** | 某 Repository 在某 Device 上的绝对路径 | `(repository, device)` 唯一；被 Agent 默认值、活跃任务、Run 或 worktree 引用时禁止删除 |
| **Device** | 一台注册过的自有机器，daemon 常驻其上 | 全局资源，不属于 Workspace；1 Device — N Agent / RepositoryMount。离线时其 Agent 的任务排队不丢 |
| **Agent** | 一条配置绑定记录：workspace + device + backend(claude/codex) + model + permission + isolation + instruction，可带 defaultRepository | 归属恰好 1 个 Workspace 与 1 个 Device；默认 Repository 只是派活默认值，不是永久 cwd。软删除后历史 Run 引用不悬空 |
| **Conversation** | Workspace 内的对话容器，`kind: chat \| issue`。Chat=临时探索，Issue=留档任务 | 可选择 0..1 Repository；Chat 始终绑定 Agent，Issue 当前 Assignee 可空。1 Conversation — N Run，同一 Conversation 严格串行 |
...
| **Automation** | Workspace 内的 cron 定时器，触发时对指定 Agent 发 prompt | 可选择 Repository；产物模式为每次新开 Issue，或追加到固定 Conversation |
...
> 下方 SQL 是 P1 初始 schema 快照；当前实现以 `apps/harbor/src/server/db.ts` 的 v9 schema 为准。v9 新增 workspaces / repositories / repository_mounts，并在 Agent、Conversation、Run、Automation、Skill 与 prompt settings 上保存或冻结 Workspace / Repository scope；旧 `workdir` 已迁移为 RepositoryMount。
...
- `DeliveryService` 内置确定性 policy，`DeliveryProvider` 隔离外部 SCM/CD 操作。首期 `manual` Provider 要求显式确认，只记录外部事实，不伪装调用平台。
...
- Agent 固定归属 Workspace + Device，可选默认 Repository，但不永久绑定 cwd；Conversation 可选任务 Repository，Run 冻结实际 mount / execution root。
- Scheduler 与 worktree 保持单 Run 单 Repository；跨仓库需求拆成多个 Issue / Run，通过 Workspace 汇总，不给单个 coding session 多个写根。
- SQLite v9 将旧数据迁入稳定的 Personal Workspace，并把旧 Agent `workdir` 转成 Repository + mount；旧 CLI `--workdir` 继续自动注册资源。
- Web 新增 Workspace switcher、Repositories 主从页，以及 Agent / Issue / Chat / Automation 的 Repository 选择；飞书 Agent 引用支持 `workspace/agent`。
- 架构决策见 `progress/decisions/2026-07-17-harbor-workspace-repository-scope.md`。
...
1. ✅ worktree 分支的交付流已在 P4.13 定案：Harbor 内置 Delivery policy，外部 SCM/CD 走 Provider；当前 manual Provider 跑通闭环，下一步按真实仓库接 Codebase/GitHub Provider。Agent/Skill 可以请求动作，但不能绕过 policy。
