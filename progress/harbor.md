...
- merge 已完成且 human approval / Harbor checks 通过后，server 幂等 enqueue SQLite durable job。job 冻结 implementation generation、target fingerprint、exact merged revision 与 lease token；重试创建新 generation，旧 revision、旧 lease、重复/stale callback 均不能覆盖当前 Delivery。
- 独立 `harbor-deploy-worker` 以最小 env 执行 exact revision checkout/build/test；cutover 前冻结原 rollback anchor并持久化 DB + 0600 host sentinel maintenance gate。只有 exact launchd label/PID 已可靠停止才允许 backup/替换 DB/plist/symlink；新服务必须通过 exact job/revision/fingerprint + label/live PID health。
- maintenance 期间 REST/WS/automation/daemon 写入路径全部拒绝，唯一放行 revision-aware health。失败使用原 attempt anchor 回滚；任何 stop/restore/health/crash identity 不确定性都进入 needs recovery，禁止普通 Retry/Done，只允许管理员 recovery CLI 实际验证旧 baseline 后解闸。
- SQLite v15 从 v13 无损迁移，并把缺 fingerprint/anchor 的 active v14 job fail-close；Web 展示 target、queued/running/succeeded/failed/needs recovery、失败原因、Retry 与 recovery 指令。无 target 时 manual fallback 完全保留。
- 测试只用 fake FS/process/launchd/HTTP/clock 和隔离 SQLite，覆盖 server restart、duplicate/stale callback、health rollback/stale 2xx、bootout/ambiguous PID、DB restore failure、healthy/rollback/terminal crash、并发 mutation 拒绝、path/env/log 安全与 manual fallback；未操作本机 launchd、真实 DB 或真实 YAML。
