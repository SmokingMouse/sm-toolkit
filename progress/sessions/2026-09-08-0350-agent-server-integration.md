...
- 验收报非零退出，本地第 3 次完整复跑复现 `daemon-pty.test.ts` 的 `prompt reached engine` 超时。Herdr 的 OSC 标题先于 raw 输入就绪，测试误用 thread id 判 ready，过早发送的 CR 被 PTY 转成 LF（Ctrl-J）。
- 仅调整测试同步，等待真实首帧及完整输入行；没有删用例、改命令/键位或生产行为。修前日志和修后逐次退出码证据均在契约 out/d3-*.log；根因与判定命令归档到 `progress/failures.md`。
- 定向连续 10 次通过，D3 全套连续 5 次均 exit 0（每次 87 pass / 0 fail）；指定 env-i 同样 87 pass，typecheck 通过。Next：交主控重新验收，证据见更新后的 out/result.md。
