...
实施状态（2026-07-19）：代码、fixtures、dry-run report、全量回归与生产 v22→v23 exact-revision 部署已完成。生产为 Account=1 / Membership=1，legacy Membership ID 保持不变，integrity=ok、FK=0，Web 登录与 bootstrap 状态接口已上线，两台 legacy-token Device 仍在线且 daemon credential 未改。尚需用户在生产 origin 完成一次 Touch ID first-owner Passkey ceremony并保存 10 个一次性 recovery codes；这是进入 P6.2 前唯一剩余步骤。
...
6. [ ] 用户在生产 HTTPS origin 完成 first-owner Passkey并保存 recovery codes。

第 6 项完成后进入 P6.2 Device ownership/enrollment；届时才迁移 daemon credential，继续避免把 Web 身份迁移和双机换证耦合在同一回滚面。
...
- 下一轮第一步是什么？——先完成生产 first-owner Passkey ceremony；验证成功后从 P6.2 Device ownership/enrollment 开始迁移 daemon credential。
...
- `apps/harbor/src/server/db.ts`：当前 v23 schema 与 identity normalization migration。
