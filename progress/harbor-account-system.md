...
### P6.2 Device ownership and enrollment（schema v25）
...
### P6.3 Agent ACL and Automation principal（schema v26）
...
### P6.4 UI, external identity and audit（schema v27）
...
P6.1 server-side 验收已完成，用户确认 recovery-code custody 后完成运营交接。独立 Device list 性能修复占用 schema v24，因此 P6.2 Device ownership/enrollment 从 v25 开始；届时才迁移 daemon credential，继续避免把 Web 身份迁移和双机换证耦合在同一回滚面。
...
- 为什么不一次完成 v23–v27？——身份、Device 换证、Agent ACL 各自有独立回滚面，分期能保持生产可用和可证伪。
- 下一轮第一步是什么？——从 schema v25 的 P6.2 Device ownership/enrollment 开始；该阶段才迁移 daemon credential。
...
- `apps/harbor/src/server/db.ts`：当前 v24 schema、identity normalization 与 Device summary migration。
