...
### P6.2 Device ownership and enrollment（schema v24）
...
### P6.3 Agent ACL and Automation principal（schema v25）
...
### P6.4 UI, external identity and audit（schema v26）
...
下一轮只做 P6.1，不同时改 Device/ACL：

1. 新起独立 worktree 与 feature branch。
2. 先写 v22 migration fixtures 和 identity normalization dry-run report。
3. 落 Account/AuthIdentity/Session/Membership/Invitation/PAT store 与 protocol。
4. 统一 REST `PrincipalContext`，保留 system token 兼容闸。
5. 完成 bootstrap/login/Workspace switcher，再跑全量回归。
6. 更新本方案的实际 schema/version 和迁移证据后，提交 Review。

P6.1 未完成前不改 daemon credential，避免一次把 Web 登录、数据库身份迁移和双机连接三个高风险面耦合。
...
- 为什么不一次完成 v23–v26？——身份、Device 换证、Agent ACL 各自有独立回滚面，分期能保持生产可用和可证伪。
- 下一轮第一步是什么？——v22 migration fixture + P6.1 identity normalization，不碰 daemon。

## 17. 参考锚点

- `progress/harbor.md`：现有 Harbor 总体架构与阶段记录。
- `progress/glossary.md`：领域术语单一真相源。
- `apps/harbor/src/protocol.ts`：当前 Device、Workspace、Agent、WorkspaceMember 类型。
- `apps/harbor/src/server/db.ts`：当前 v22 schema 与 v13 Workspace RBAC migration。
- `apps/harbor/src/server/rest.ts`：当前 system/member actor、Workspace role 和 Agent visibility 实现。
